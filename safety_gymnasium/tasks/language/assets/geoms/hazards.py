...
from safety_gymnasium.tasks.language.assets.group import GROUP
from safety_gymnasium.tasks.language.bases.base_object import Geom
...
class Hazards(Geom):  # pylint: disable=too-many-instance-attributes
    ...
    def cal_cost(self):
        ...
        for index in range(self.agents.num):
            ...
            cost_index['cost_hazards'] = 0
            for h_pos in self.pos:
                h_dist = self.agents.dist_xy(h_pos, index)
                # pylint: disable=no-member
                if h_dist <= self.size:
                    cost_index['cost_hazards'] += self.cost * (self.size - h_dist)

...
from safety_gymnasium.tasks.language.bases.underlying import Underlying
from safety_gymnasium.tasks.language.utils.common_utils import ResamplingError
from safety_gymnasium.tasks.language.utils.task_utils import theta2vec
...
class BaseTask(Underlying):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def build_goal_position(self) -> None:
        ...
        elif  self.goal_achieved.any() and 'language'  in self.__class__.__name__.lower():
            print("hhaha")
            for i in range(self.goals.num):
                self.world_info.world_config_dict['geoms'][f'goal{i}']['pos'][:2] = self.world_info.layout[f'goal{i}']
                self._set_goal('goals', self.world_info.layout[f'goal{i}'])
